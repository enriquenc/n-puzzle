...
class Visualization(Canvas):
    def __init__(self, puzzle_solving, size, master=None, bg="white"):
        ...
        self.size = size
        self.colors = ["blue", "white", "red", "green", "cyan", "yellow", "magenta"]
        self.current_color = 0
        self.auto = False
        self.index = 0
        self.print_puzzle(puzzle_solving[self.index])
        self.master.bind('<plus>', self.change_color)
    ...
    def change_color(self, button=None):
    ...
    def button_handler(self, button=None):
        if button is not None and self.auto is True:
            return
